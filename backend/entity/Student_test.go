...
func Test_Validate_Student_Number_NotNull(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	// err.Error ต้องมี error message แสดงออกมา
	g.Expect(err.Error()).To(Equal("Student_Number: non zero value required"))
}
...
func Test_Validate_Student_Identity_Card_NotNull(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	// err.Error ต้องมี error message แสดงออกมา
	g.Expect(err.Error()).To(Equal("Student_Identity_Card: non zero value required"))
}
